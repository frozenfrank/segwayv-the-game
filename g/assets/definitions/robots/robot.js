function robot() {
	return updateSome(new userObject, {
		...
		gameCycle: function() {
			...
			//do something different when the user is dead
			var currentState = AI.user.user.gamePlay.isDead ? 'userIsDead' : this.ai.currentState();
			...
			if (this.ai.states[currentState]) //update: thats stupid!
		...
		collideWith: function(otherOBJ) {
			...
			switch (otherOBJ.class) {
				...
				case 'userObject':
					//damage computations
					if(!thatg.isDead)
						//this will fix the increasing health when the user is dead
		...
		spawn: function() {
			...
			if (c.total + 1 > AI.config.maxRobots) return;
			...
			var robot = functions.objectFactory(what, {
				appearance: {
		...
		save: function() {
			...
			c[this.name]++;
			c.total++;
			c.totalValue += this.gamePlay.robotValue;
		},
